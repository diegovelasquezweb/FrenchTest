  ...
  return (
    <div className="mx-auto w-full max-w-xl space-y-6">
      <div className="rounded-(--radius-card) bg-(--color-surface) p-6 text-center shadow-sm sm:p-8">
        <h2 className="text-2xl font-bold text-(--color-ink)">Quiz terminé</h2>
        <p
        ...
        <ol className="space-y-2">
          {history.map((entry, i) => (
            <li
              ...
              aria-label={`${entry.verb.infinitive} : vous avez répondu ${entry.picked}, bonne réponse : ${entry.verb.participle}`}
            >
              <span
                aria-hidden="true"
