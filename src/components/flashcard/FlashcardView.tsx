...
export function FlashcardView({
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown" || e.key === " " || e.key === "4") {
        e.preventDefault();
        triggerSkip();
      }
      if (e.key === "Backspace" || e.key === "Escape") {
  ...
  const isPlaceholderPair =
    normalizedEn === normalizedFront && normalizedEs === normalizedFront;
  const hasEnTranslation =
    card.translationEn.trim().length > 0 && !isPlaceholderPair;
  const hasEsTranslation =
    card.translationEs.trim().length > 0 && !isPlaceholderPair;
      ...
      <div className="w-full max-w-xl">
        <div className="mb-4 flex items-center justify-center gap-3">
          ...
          <Popover.Root>
            ...
            <Popover.Portal>
              ...
              >
                <p className="mb-2.5 text-[10px] font-semibold uppercase tracking-widest text-muted/60">
                  Raccourcis
                </p>
                  ...
                  {(
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`inline-flex h-5 w-5 items-center justify-center rounded text-[11px] font-bold ${bg} ${color}`}
                        >
                          {gesture}
                        </span>
                      ...
                      <div className="hidden md:flex items-center gap-1">
                        {keys.map((k) => (
                          <kbd
                            key={k}
                            className="inline-flex h-5 min-w-5 items-center justify-center rounded border border-ink/12 bg-ink/5 px-1 text-[10px] font-medium text-muted"
                          >
                            {k}
                          </kbd>
                        ))}
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <span
              className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${CATEGORY_COLOR[card.category]}`}
            >
              ...
              <div className="mt-4 flex flex-col gap-1 border-t border-ink/8 pt-3">
                {hasEnTranslation && (
                  <p className="text-xs text-muted" lang="en">
                    <span className="mr-1.5 inline-flex rounded bg-ink/8 px-1.5 py-0.5 text-[10px] font-semibold tracking-wide text-ink">
                      ENG
                    </span>
                    {card.translationEn}
                  </p>
