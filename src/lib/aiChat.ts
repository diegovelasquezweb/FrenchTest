...
export async function checkAiKey(): Promise<boolean> {
  ...
  try {
    const res = await fetch(`${WORKER_URL}/aikey`, { headers: JSON_HEADERS, credentials: "include" });
...
export async function deleteAiKey(): Promise<void> {
  ...
  await fetch(`${WORKER_URL}/aikey`, { method: "DELETE", headers: JSON_HEADERS, credentials: "include" });
}
