...
interface FuturQuizState {
  phase: QuizPhase;
  questions: FuturQuestion[];
  currentIndex: number;
  score: number;
...
const initialState: FuturQuizState = {
...
function reducer(state: FuturQuizState, action: FuturAction): FuturQuizState {
  switch (action.type) {
    case "HOME":
      return { ...initialState };
    case "START":
    case "RESTART":
      return {
        ...
        questions: buildFuturQuestions(VERBS, 10, Math.random),
      };
    case "SELECT": {
...
export interface UseFuturQuizReturn {
  state: FuturQuizState;
  startQuiz(): void;
  selectAnswer(index: number): void;
  nextQuestion(): void;
  restartQuiz(): void;
  goHome(): void;
  currentQuestion: FuturQuestion | null;
  progress: { index: number; total: number };
}

export function useFuturQuiz(): UseFuturQuizReturn {
