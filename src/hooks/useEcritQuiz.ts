...
const initialState: EcritQuizState = {
...
function buildQuestions(count: number): OrthographeQuestion[] {
  const shuffled = fisherYates([...PHRASES_ECRIT], Math.random);
...
function reducer(state: EcritQuizState, action: EcritAction): EcritQuizState {
  switch (action.type) {
    case "HOME":
      return { ...initialState };
    case "START":
...
export interface UseEcritQuizReturn {
  state: EcritQuizState;
  startQuiz(): void;
...
export function useEcritQuiz(): UseEcritQuizReturn {
