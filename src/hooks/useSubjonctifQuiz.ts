...
interface SubjonctifQuizState {
  phase: QuizPhase;
  questions: SubjonctifQuestion[];
  currentIndex: number;
  score: number;
  selectedIndex: number | null;
  answerState: AnswerState;
  history: { question: SubjonctifQuestion; picked: string; correct: boolean }[];
  everWrong: boolean;
}

type SubjonctifAction =
  | { type: "START" }
...
const initialState: SubjonctifQuizState = {
...
function buildQuestions(count: number): SubjonctifQuestion[] {
...
function reducer(state: SubjonctifQuizState, action: SubjonctifAction): SubjonctifQuizState {
  switch (action.type) {
    case "HOME":
      return { ...initialState };
    case "START":
...
export interface UseSubjonctifQuizReturn {
  state: SubjonctifQuizState;
  startQuiz(): void;
  selectAnswer(index: number): void;
  nextQuestion(): void;
  restartQuiz(): void;
  goHome(): void;
  currentQuestion: SubjonctifQuestion | null;
  progress: { index: number; total: number };
}

export function useSubjonctifQuiz(): UseSubjonctifQuizReturn {
