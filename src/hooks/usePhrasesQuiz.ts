...
const initialState: PhrasesQuizState = {
...
function buildQuestions(count: number): OrthographeQuestion[] {
  const shuffled = fisherYates([...PHRASES_DISCOURS], Math.random);
...
function reducer(state: PhrasesQuizState, action: PhrasesAction): PhrasesQuizState {
  switch (action.type) {
    case "HOME":
      return { ...initialState };
    case "START":
...
export interface UsePhrasesQuizReturn {
  state: PhrasesQuizState;
  startQuiz(): void;
...
export function usePhrasesQuiz(): UsePhrasesQuizReturn {
