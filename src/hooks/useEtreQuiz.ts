...
const initialState: EtreQuizState = {
...
function buildQuestions(count: number): OrthographeQuestion[] {
  const shuffled = fisherYates([...PHRASES_ETRE], Math.random);
...
function reducer(state: EtreQuizState, action: EtreAction): EtreQuizState {
  switch (action.type) {
    case "HOME":
      return { ...initialState };
    case "START":
...
export interface UseEtreQuizReturn {
  state: EtreQuizState;
  startQuiz(): void;
...
export function useEtreQuiz(): UseEtreQuizReturn {
