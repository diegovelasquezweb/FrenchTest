...
interface PlusQueParfaitQuizState {
  phase: QuizPhase;
  questions: PlusQueParfaitQuestion[];
  currentIndex: number;
  score: number;
  selectedIndex: number | null;
  answerState: AnswerState;
  history: { question: PlusQueParfaitQuestion; picked: string; correct: boolean }[];
  everWrong: boolean;
}

type PlusQueParfaitAction =
  | { type: "START" }
...
const initialState: PlusQueParfaitQuizState = {
...
function buildQuestions(count: number): PlusQueParfaitQuestion[] {
...
function reducer(state: PlusQueParfaitQuizState, action: PlusQueParfaitAction): PlusQueParfaitQuizState {
  switch (action.type) {
    case "HOME":
      return { ...initialState };
    case "START":
    ...
    case "SELECT": {
      ...
      return {
        ...
        history: [
          ...state.history,
          { question, picked: question.options[action.payload] ?? "", correct: !state.everWrong },
        ],
      };
    }
    case "NEXT": {
...
export interface UsePlusQueParfaitQuizReturn {
  state: PlusQueParfaitQuizState;
  startQuiz(): void;
  selectAnswer(index: number): void;
  nextQuestion(): void;
  restartQuiz(): void;
  goHome(): void;
  currentQuestion: PlusQueParfaitQuestion | null;
  progress: { index: number; total: number };
}

export function usePlusQueParfaitQuiz(): UsePlusQueParfaitQuizReturn {
